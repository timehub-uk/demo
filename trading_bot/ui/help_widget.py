...
<p>Open <code>Settings → Binance</code> and enter your Binance API Key and Secret.
...
<h3>3. AutoTrader</h3>
<p>The AutoTrader scans all USDT pairs every 5 minutes using the full ML stack
(LSTM + Transformer ensemble, MTF confluence, signal council, regime detector).
In <code>SEMI_AUTO</code> mode, press <b>🎯 Take Aim</b> to approve the recommended trade.
In <code>FULL_AUTO</code> mode, trades fire when confidence ≥ threshold.</p>
...
<h3>Data Flow</h3>
<p><code>Binance WebSocket</code> → <code>Redis Cache</code> → <code>ML Predictor</code>
→ <code>Ensemble</code> → <code>Signal Council</code> → <code>AutoTrader</code>
...
<ul>
  ...
  <li>Trade timeout (auto-close): <b>4 hours</b></li>
</ul>
...
<h2>SUPPORT</h2>
<p>Intel Log (<code>Ctrl+L</code>) shows all real-time activity.
...
class HelpWidget(QWidget):
    ...
    def _build_about_tab(self) -> None:
        ...
        features = [
            ...
            (GREEN,  "Autonomous AutoTrader with SEMI/FULL-AUTO modes"),
            (GREEN,  "Market scanner – all USDT pairs, parallel scoring"),
